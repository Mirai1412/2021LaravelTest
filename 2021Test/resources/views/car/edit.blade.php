    ...
    <body>
        <x-slot name="header">
            <div class="flex justify-between">
                ...
                <button onclick=location.href="{{ route('show', ['car'=>$car->id]) }}" type="button" class="btn btn-info hover:bg-blue-700 font-bold text-white">
                    상세보기
                </button>
            </div>
        </x-slot>
        <div class="m-4 p-4">
            <form id="editForm" class="row g-3" action="{{ route('update', ['car'=>$car->id]) }}"
                        method="post"
                ...
                <div class="col-12 m-2">
                    ...
                    name="content"
                    id="content">{{$car->company }}</textarea>
                    @error('content')
                    <div class="text-red-800">
                        <span>{{ $message }}</span>
                    </div>
